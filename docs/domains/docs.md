---
status: stable
purpose: Define governance constraints for the docs domain.
governed_by:
  docs/reference/docs/system-governance.md: System governance
governs:
  docs/how-to/docs/content-aware-restructuring.md: Restructuring procedure
  docs/explanation/docs/system-rationale.md: System rationale
  docs/tutorials/getting-started.md: System onboarding
---

# Docs Domain

## Purpose / Scope
Documentation system structure, routing, and maintenance.

## Governance Constraints
- Docs must be linked through `governs` and `governed_by`.
- Canonical docs live in `reference`, `how-to`, or `explanation`.
- `tutorials` are learning only and lowest authority.

## Escalation / Awareness Triggers
None

## Cross-Domain Interfaces
None
